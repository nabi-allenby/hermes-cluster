...
| `drills/0-substrate/` | prove the agent-sandbox substrate: claim → suspend → PVC survives → resume (`make drill-substrate`) | minikube |
| `drills/1-agent-session/` | the real hermes-agent as a session: boot → self-provision → connect → suspend → buffered wake → drain (`make drill-agent`) | + agent image, connector image, seed home (`make-seed.sh`) |
...
Measured timings from these drills: `docs/architecture.md` (measured
reference) and `docs/agent-sandbox.md`.
